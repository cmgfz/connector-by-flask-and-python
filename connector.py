...
def getByCategory(category):
    ...
    if(category.find("快餐"))!=-1:
        return jsonify(category_shop)
    else:
        return jsonify({"message":"您输入的内容有误"})
...
def getByKey():
    ...
    with open('shops.json','r',encoding='utf-8') as f:
        ...
        shops = json.loads(a)
        for shop in shops:
            if(shop["name"].find(keyword)!=-1 and shop['latitude']==float(latitude) and shop['longitude']==float(longitude)): 
                copy.append(shop)
    print(copy)
    key_shop={
        "code":0,
...
def login():
    ...
    # 这样获取就可以了 
        ...
        if( isinstance(json_data["name"], str)  and \
           isinstance(json_data["pwd"], str) and json_data["captcha"]=="wk3v"):
            result={
                "code": 0,
                "data": {
        ...
        print(isinstance(json_data["name"],str))
        print(isinstance(json_data["pwd"],str))
...
def loginSms():
    ...
    # 这样获取就可以了 
        json_data = request.json
        if(re.match('[0-9]{11}',json_data['phone']) \
           and json_data["code"]=="wk3v"):
            result= {
                "code": 0,
                "data": {
                  "_id": "5a9cd9c6ad5b2d34d42b385d",
